...
function parseMarkdown(filepath) {
  ...
  for (let i = 2; i < lines.length; i++) {
    ...
    if (line.startsWith('# ')) {
    ...
    } else if (line.startsWith('`') && tags.length === 0) {
      tags = line.match(/`([^`]+)`/g).map(t => t.replace(/`/g, ''));
      ...
      if (line.trim().length > 0 || currentSection.body.length > 0) {
  ...
  // Clean up bodies
  sections.forEach(s => {
    s.body = s.body.trim().replace(/---/g, '').trim();
  });
  
  return { title, date, readTime, tags, sections, references };
}

const p1 = parseMarkdown('blog-position-bias.md');
const p2 = parseMarkdown('blog-redteaming.md');

const allBlogs = [p1, p2, currentBlog];

const fileContent = `export const blogPosts = ${JSON.stringify(allBlogs, null, 2)};\n`;
